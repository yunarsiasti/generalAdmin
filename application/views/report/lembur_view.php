			...
			<h1 class="h2">Report Lembur Pegawai</h1>
			</div>
			
			<?php
			echo form_open('KelolaLembur/simpanLembur');//echo"<pre>";print_r($pegawai);echo"</pre>";die();
			?>
			...
			<tr>
				...
				<td>
				<div class="col-sm-4"">
					<input type="hidden" name="end_time" class="form-control" value="-">
			...
			<tr>
				...
				<?php echo anchor('KelolaLembur/showMasterLembur','Kembali',array('class'=>'btn btn-primary btn-sm'))?>
				</td>
			</tr>
			</table>
			</form>
			...
			<tbody>
				...
				foreach ($kehadiran_pegawai as $r)
				{
					...
					if($r->id_jabatan==1 OR $r->id_jabatan==2 OR $r->id_jabatan==3 OR $r->id_jabatan==5){
						echo "
						<td>".anchor('KelolaLembur/editLemburSetuju/'.$r->id_kehadiranPegawai,'Setuju')."</td>
						<td>".anchor('KelolaLembur/editLemburTidakSetuju/'.$r->id_kehadiranPegawai,'TidakSetuju')."</td>
						<td>".anchor('KelolaLembur/hapusLembur/'.$r->id_kehadiranPegawai,'Hapus')."</td>
						<td></td></tr>
						";
					}else{
						echo"
						<td>".anchor('KelolaLembur/editLemburSetuju/'.$r->id_kehadiranPegawai,'Setuju')."</td>
						<td>".anchor('KelolaLembur/editLemburTidakSetuju/'.$r->id_kehadiranPegawai,'TidakSetuju')."</td>
						<td>".anchor('KelolaLembur/editLemburMenunggu/'.$r->id_kehadiranPegawai,'Menunggu')."</td>
						<td>".anchor('KelolaLembur/hapusLembur/'.$r->id_kehadiranPegawai,'Hapus')."</td></tr>
						";
					}
					$no++;
				}
